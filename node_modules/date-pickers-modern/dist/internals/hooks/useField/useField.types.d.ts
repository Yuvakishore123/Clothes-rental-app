...
export type UseFieldResponse<TForwardedProps extends UseFieldForwardedProps> = Omit<TForwardedProps, keyof UseFieldForwardedProps> & Required<UseFieldForwardedProps> & Pick<React.HTMLAttributes<HTMLInputElement>, 'autoCorrect' | 'inputMode'> & {
...
export type FieldBoundaries<TDate, TSection extends FieldSection> = Record<MuiDateSectionName, (currentDate: TDate | null, section: TSection) => {
    minimum: number;
    maximum: number;
}>;
...
export type FieldSelectedSections = number | MuiDateSectionName | null | 'all' | {
    startIndex: number;
    endIndex: number;
};
